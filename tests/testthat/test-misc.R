test_that("calc_time_in_state_diff returns all observed states by default", {
  ...
  expect_equal(
    result$reference_sd_time,
    c(stats::sd(c(1, 2)), stats::sd(c(2, 1)))
  )
  expect_equal(
    result$treatment_sd_time,
    c(stats::sd(c(0, 1)), stats::sd(c(3, 2)))
  )
})
...
test_that("state conversion helpers validate required columns", {
  expect_error(
    states_to_ttest(data.frame(id = 1)),
    "data must contain columns: y, tx"
  )
  expect_error(
...
test_that("states_to_ttest preserves character treatment labels", {
  data <- data.frame(
    id = c(1, 1, 1, 2, 2, 2),
    y = c(1, 2, 1, 2, 2, 1),
    tx = factor(
  ...
  expect_equal(
    result$tx,
    c("Placebo + Remdesivir", "Baricitinib + Remdesivir")
  )
})
...
test_that("states_to_drs preserves character treatment labels", {
  data <- data.frame(
    id = rep(1:2, each = 4),
    time = rep(1:4, 2),
    y = c(2, 1, 1, 1, 2, 2, 1, 1),
...
test_that("calc_time_in_state_diff validates inputs", {
  ...
  expect_error(
    calc_time_in_state_diff(data.frame(id = 1)),
    "data must contain columns"
  )
  expect_error(
