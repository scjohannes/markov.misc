...
#' @param Gamma Effective coefficient matrix [M x P]
...
markov_msm_run <- function(components, Gamma, times, absorb = NULL) {
  ...
  absorb_idx <- if (!is.null(absorb)) which(ylevels %in% absorb) else integer(0)
...
#' @inheritParams soprob_markov_fast
#'
#' @export
...
#' 
#' @param eta Matrix of linear predictors [N x M]
#' @param M Number of thresholds
#' @return Matrix of probabilities [N x (M+1)]
