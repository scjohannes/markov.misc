...
plot_comparisons <- function(
  ...
  geom <- match.arg(geom)
  plot_sops_validate_scalar(ribbon_alpha, "ribbon_alpha", lower = 0, upper = 1)
  plot_sops_validate_scalar(point_size, "point_size", lower = 0)
  plot_sops_validate_scalar(line_width, "line_width", lower = 0)
  if (
    !is.logical(show_uncertainty) ||
      length(show_uncertainty) != 1 ||
      is.na(show_uncertainty)
  ...
  plot_sops_validate_columns(data, x_var, "`x_var`")
  plot_sops_validate_facets(data, facet_var)

  if (is.null(color_var)) {
    color_var <- plot_comparisons_default_color_var(data, x_var)
  } else {
    plot_sops_validate_columns(data, color_var, "`color_var`")
  }
