...
for (i in effect_sizes) {
  ...
  # 3. Time-to-Event format (Anderson-Gill)
  count_data <- states_to_tte(markov_data, covariates = NULL)

  # --- Prepare Data for Markov Analysis ---
  # Removes absorbing states and converts variables
  ...
  write_parquet(count_data, file.path(DATA_PATH, paste0("superpopulation_count_", es_label, ".parquet")))
}
```
...
# Using finegray subdistributional hazard ratio
fit_time_to_discharge <- function(data, iter) {
  data <- format_competing_risks(data, event_status = 1, death_status = 6) |>
    mutate(
      status = factor(
        status,
        levels = c(0, 1, 2),
        labels = c("censored", "discharge", "death")
      )
    )
  ...
  fg_model <- survival::coxph(
    survival::Surv(fgstart, fgstop, fgstatus) ~ tx + yprev,
    data = fg_data,
    weights = fgwt,
    cluster = id
  )

  tidy_po(fg_model, alternative = "two.sided") |>
    filter(term == "tx") |>
    mutate(
      iter = iter,
      analysis = "time_to_discharge_fg",
      se_type = "robust",
      .before = 1
    )
}

fit_time_to_recovery <- function(data, iter) {
  # Restructure data
  # data$lead_dur <- lead(data$stop - data$start)
  data <- data |>
    group_by(id) |>
    mutate(lead_dur = lead(stop - start)) |>
    ungroup()

  data$event <- data$y == 1 & data$lead_dur > 3
  data$event <- ifelse(is.na(data$event), FALSE, data$event)
  split_data <- split(data, data$id)
  temp <- lapply(split_data, FUN = function(x) {
    if (any(x$event == TRUE)) {
      x[x$event & x$stop == min(x$stop[x$event]), ]
    } else {
      x[nrow(x), ]
    }
  })
  ...
  cox_model <- survival::coxph(
    survival::Surv(time = stop, event = event) ~ tx + yprev,
    #cluster = id, # Check the type-I-error without robust SEs
    data = data
  )

  tidy_po(cox_model, alternative = "two.sided") |>
    filter(term == "tx") |>
    mutate(
      iter = iter,
      analysis = "time_to_recovery_cox",
      se_type = "naive",
      .before = 1
    )
}
...
fit_functions <- list(
  ...
  drs_orm = fit_drs_orm,
  ttest = fit_ttest,
  time_to_discharge = fit_time_to_discharge,
  time_to_recovery = fit_time_to_recovery,
...
for (es_val in available_effect_sizes) {
  ...
  data_paths <- list(
    ...
    time_to_discharge = file.path(DATA_PATH, paste0("superpopulation_count_", es_label, ".parquet")),
    time_to_recovery = file.path(DATA_PATH, paste0("superpopulation_count_", es_label, ".parquet")),
    binary_death = file.path(DATA_PATH, paste0("superpopulation_count_", es_label, ".parquet"))
  )
