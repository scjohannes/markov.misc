...
m_flex <- vglm(
  ordered(y) ~ (time_lin + time_nlin_1 + time_nlin_2) + tx + yprev + yprev:time_lin + tx:time_lin, 
...
set.seed(SEED)
t1 <- Sys.time()
bs_simple_mvn <- run_mvn(m_simple, "Model A")
t2 <- Sys.time()

t2 - t1

bs_inter_mvn  <- run_mvn(m_inter,  "Model B")
bs_flex_mvn   <- run_mvn(m_flex,   "Model C")
```

```{r}
sops(
     robcov_vglm(m_simple, cluster = data$id), #model needs to have cluster robust SEs
     newdata = data |> filter(time == 1), 
     times = 1:FU,
     ylevels = 1:6,
     absorb = "6",
  ...
  ggplot(aes(
    ...
    y = estimate, 
    linetype = factor(tx), 
    # Combine tx and state into a single interaction for grouping
    group = interaction(factor(tx), factor(state)),
    color = factor(state) 
  )) +
  geom_line() + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = factor(state), group = interaction(factor(tx), factor(state))), alpha = 0.2, color = NA) +
  facet_wrap(~yprev)
```
...
(p1_mvn | p2_mvn | p3_mvn) +
  plot_layout(guides = "collect") +
  patchwork::plot_annotation(title = "MVN")
```
